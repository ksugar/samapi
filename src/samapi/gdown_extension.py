"""
This module is a modified version of gdown, in which the download is cancellable.
Original code: https://github.com/wkentaro/gdown/blob/0080deee511da05020cf863bec44bf2c0fd08715/gdown/download.py
"""
...
def download(
    ...
    format: str, optional
        Format of Google Docs, Spreadsheets and Slides. Default is:
            ...
            - Google Slides: 'pptx'
    cancel_filepath: str, optional
        Path to a file that, if it exists, signals download cancellation.
    ...
    try:
        ...
        for chunk in res.iter_content(chunk_size=CHUNK_SIZE):
            if cancel_filepath is not None and os.path.exists(cancel_filepath):
                sys.stderr.write(
                    'Cancelled download, removing temporary file "{}"'.format(f.name)
                )
                sys.stderr.flush()
                if tmp_file:
                    f.close()
